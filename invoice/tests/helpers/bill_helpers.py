...
def create_test_bill(subject=None, sender=None, user=None, **custom_props):
    subject = subject or __create_test_subject()
    sender = sender or __create_test_sender()
    payload = DEFAULT_TEST_BILL_PAYLOAD.copy()
    payload['subject'] = subject
    payload['subject_type'] = ContentType.objects.get_for_model(subject)
    payload['sender'] = sender
    payload['sender_type'] = ContentType.objects.get_for_model(sender)
